...
const { FALLBACK_CATALOG, getCommercialCatalog } = require('./commercial-catalog');

function formatNis(agorot) {
  const nis = agorot / 100;
  return nis.toLocaleString('en-US', {
    minimumFractionDigits: Number.isInteger(nis) ? 0 : 2,
    maximumFractionDigits: 2,
  });
}

function buildSystemPrompt(catalog = FALLBACK_CATALOG) {
  const prices = catalog.prices;
  const skus = catalog.skus || FALLBACK_CATALOG.skus;
  const starterMonthly = formatNis(prices.starter.monthly);
  const proMonthly = formatNis(prices.pro.monthly);
  const enterpriseMonthly = formatNis(prices.enterprise.monthly);
  const starterAnnual = formatNis(prices.starter.annual);
  const proAnnual = formatNis(prices.pro.annual);
  const enterpriseAnnual = formatNis(prices.enterprise.annual);
  const operationsMonthly = formatNis(prices.operations.monthly);
  const operationsAnnual = formatNis(prices.operations.annual);
  const operationsMonthlyEquivalent = formatNis(Math.round(prices.operations.annual / 12));
  const operationsAnnualSavings = formatNis((prices.operations.monthly * 12) - prices.operations.annual);
  const pack1 = formatNis(skus.pack_1);
  const pack3 = formatNis(skus.pack_3);
  const pack10 = formatNis(skus.pack_10);
  const extraProj10 = formatNis(skus.extra_proj_10);
  const extraProj25 = formatNis(skus.extra_proj_25);
  const extraUsers10 = formatNis(skus.extra_users_10);
  const extraUsers50 = formatNis(skus.extra_users_50);

  return `אתה "SitePunch AI" — עוזר וירטואלי בעמוד הנחיתה של משפחת מוצרי SitePunch לאתרי בנייה.
...
תמחור SitePunch: Free ₪0 לצמיתות (פרויקט אחד, עד 3 משתמשים, תיעוד + PDF בסיסי, ללא כרטיס אשראי) · Starter ₪${starterMonthly}/חודש (עד 3 פרויקטים, 10 משתמשים, PDF מקצועי, ייצוא Excel/CSV) · Pro ₪${proMonthly}/חודש — הכי פופולרי (עד 100 פרויקטים, 50 משתמשים, PDF עם לוגו, תוכניות קומה, דשבורד מנהל, הרשאות לפי תפקיד, תמיכה מועדפת והתראות תאריך יעד במייל לפי בחירת המשתמש) · Enterprise ₪${enterpriseMonthly}/חודש + ₪990 הקמה (ללא הגבלת פרויקטים, עד 200 משתמשים, עוזר AI בבטא, מיתוג ארגוני, אינטגרציות לפי אפיון, ליווי אישי ותמיכה מועדפת). מחירי חיוב שנתי: Starter ₪${starterAnnual}, Pro ₪${proAnnual}, Enterprise ₪${enterpriseAnnual}. אפשר גם ללא מנוי: פרויקט בודד ₪${pack1} (3 חודשים, עד 10 משתמשים), חבילת 3 פרויקטים ₪${pack3} (6 חודשים), חבילת 10 פרויקטים ₪${pack10} (שנה). חבילות אקסטרה: +10 פרויקטים ₪${extraProj10}/חודש, +25 פרויקטים ₪${extraProj25}/חודש, +10 משתמשים ₪${extraUsers10}/חודש, +50 משתמשים ₪${extraUsers50}/חודש (Enterprise).
תמחור SitePunch Operations: מחיר מחירון ₪${operationsMonthly} לחודש או ₪${operationsAnnual} לשנה, כלומר ₪${operationsMonthlyEquivalent} לחודש בחיוב שנתי וחיסכון שנתי של ₪${operationsAnnualSavings}. זהו מוצר נפרד ואינו כלול ב-Enterprise. אפיון, הטמעה ושירותים מותאמים מוצעים בנפרד ובכתב מראש. כאשר נשאלים על המחיר, יש לנקוב בסכום המדויק מן הקטלוג ולא לנסח אותו כ״מעל״, כ״החל מ־״ או כטווח. אין להמציא מגבלות משתמשים או פרויקטים ל-Operations אם אינן מופיעות בעובדות האלה.
...
const SYSTEM_PROMPT = buildSystemPrompt(FALLBACK_CATALOG);
...
module.exports = async (req, res) => {
  ...
  try {
    const systemPrompt = buildSystemPrompt(await getCommercialCatalog());
